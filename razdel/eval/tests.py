...
def generate_partition_precision_tests(partition):
    for substring in partition.as_substrings:
...
def correct_precision(segment, partition):
    etalon = list(partition.as_substrings)
    assert len(etalon) == 1
    guess = list(segment(partition.as_text))
    return len(guess) == 1
...
def correct_recall(segment, partition):
    substrings = list(partition.as_substrings)
    assert len(substrings) == 2
    etalon = substrings[0].stop
    for part in segment(partition.as_text):
        if part.stop == etalon:
            return True
    return False
